...
function PlinkoCanvas({ 
  ...
  const animate = useCallback(() => {
    ...
    // Draw pegs with enhanced styling
    ...
    multipliers.forEach((multiplier, index) => {
      ...
      if (multiplier >= 8.0) {
        bgColor = '#DC2626' // Red for highest multipliers
        borderColor = '#B91C1C'
        textColor = '#FFFFFF'
      } else if (multiplier >= 4.0) {
        bgColor = '#EA580C' // Orange for high multipliers
        borderColor = '#C2410C'
        textColor = '#FFFFFF'
      } else if (multiplier >= 2.0) {
        bgColor = '#059669' // Green for good multipliers
        borderColor = '#047857'
        textColor = '#FFFFFF'
      } else if (multiplier >= 1.0) {
        bgColor = '#0D9488' // Teal for break-even
        borderColor = '#0F766E'
        textColor = '#FFFFFF'
      } else {
        bgColor = '#4B5563' // Gray for losing multipliers
        borderColor = '#374151'
        textColor = '#F3F4F6'
      }
...
function ArkConnect({ onWalletConnected, onDisconnect, connectedWallet, onBalanceUpdate }: {
  ...
  const connectWallet = async () => {
    try {
      ...
      // Convert balance to proper format if needed
      let formattedBalance = balance
      if (typeof balance === 'number') {
        formattedBalance = (balance / 100000000).toString()
      } else if (typeof balance === 'string') {
        // Check if balance is in arktoshi and convert to ARK
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md">
              <div className="p-4 border-b border-gray-200">
                <h3 's the complete updated file:
